...
- 앱 소스·런타임, 전용 `CODEX_HOME`, 로그·Snapshot·암호화 vault와 임시 작업공간은
  `<설치 폴더>` 아래에 모은다.
- 실제 업무 파일은 사용자가 명시적으로 선택한 프로젝트 폴더에서 작업한다. 설치 폴더와
  프로젝트 폴더는 서로 다른 드라이브나 여러 단계 아래의 중첩 경로여도 된다. Downloads도
  사용자가 프로젝트로 선택했을 때만 기본 작업 범위가 된다.
...
중요한 한계도 있다. 회사 호환성을 우선한 `company-direct` 프로필은 별도 Sandbox
사용자 로그온을 요구하지 않는 대신, 로컬 명령을 현재 로그인한 Windows 사용자의 권한으로
실행한다. 따라서 **프로젝트와 설치 폴더 밖의 파일·네트워크 접근을 Windows가 강제로
차단한다고 보장하지 않는다.** 위 항목은 작업 범위를 좁히고 실수를 줄이는 제품 안전장치이지
OS 샌드박스나 완전한 DLP가 아니다. 공개 웹 검색은 의도적으로 지원하므로 검색어와 대화에
넣은 내용은 Codex 서비스로 전송될 수 있다. 기밀 원문, 내부 경로, 고객 식별자, 자격증명은
프롬프트나 검색어에 포함하지 말고 조직의 승인 범위에서 사용해야 한다. 자세한 내용은
[`SECURITY.md`](SECURITY.md)와 [`README_PORTABLE.md`](README_PORTABLE.md)에 있다.
